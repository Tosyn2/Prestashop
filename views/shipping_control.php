<?php $tokenc = Tools::getAdminToken('AdminAddresses'.(int)TabCore::getIdFromClassName('AdminAddresses').(int)\ContextCore::getContext()->cookie->id_employee);?>
							...
							<?php foreach ($collectionAddresses as $collectionAddress): ?>
								<?php if ($collectionAddress['id_address'] == $collectionAddressId): ?>
									<a class="btn btn-default pull-right"
									   href="?controller=AdminManufacturers&amp;token=<?= $tokenb ?>&amp;id_address=<?= $collectionAddress['id_address'] ?>&amp;editaddresses=1">
			...
			<div style="float:right;width:49%">
				...
				<div class="well">
					<div class="row">
						<div class="col-sm-6">
							<?php foreach ($deliveryAddresses as $deliveryAddress):
								if ($deliveryAddress['id_address'] == $deliveryAddressId) {
									?>
									<a class="btn btn-default pull-right"
									   href="?controller=adminaddresses&amp;id_address=<?= $deliveryAddress['id_address'] ?>&amp;updateaddress&amp;token=<?= $tokenc ?>">
										<i class="icon-edit"></i> Edit
									</a>
									<?php
									echo
						...
						<a class="btn btn-default pull-right"
						   href="index.php?controller=AdminAddresses&addaddress&id_order=<?=$orderId?>&token=<?=$tokenc?>">
							<i class="icon-plus-circle"></i> Add Address
						</a>
					</div>
				</div>
